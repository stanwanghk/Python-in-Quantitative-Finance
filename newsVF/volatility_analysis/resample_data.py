...
def pro_one_code(code='sz000001', return_col=['code', 'p_var', 'mean_return', 'num_news', 'sum_abs_sent']):
    ...
    draw_news['sum_abs_sent'] = raw_news[
        'abs_sentiment'].resample('W', label='left').sum()
    draw_news['num_news'] = raw_news[
        'sentiment'].resample('W', label='left').count()
    # fill the NaN with 0 for news
    draw = draw.join(draw_news)
    draw[['sum_abs_sent', 'num_news']] = draw[
        ['sum_abs_sent', 'num_news']].fillna(0)
